...
pub enum Error {
    #[error(transparent)]
    TurtleError(#[from] rio_turtle::TurtleError),
    #[error(transparent)]
    IoError(#[from] std::io::Error),
    #[error(transparent)]
    RegexError(#[from] regex::Error),
    #[error("Unknown part of speech: {pos}")]
    UnknownPartOfSpeech { pos: String },
}
...
impl Dictionary {
    ...
    pub fn build<P: AsRef<Path>>(path: P) -> Result<Self> {
        Self::build_from_reader(BufReader::new(File::open(path)?))
    }
        ...
        for entry in lexical_entries {
            ...
            for sense in entry.filtered_edges(|e| e.label.contains("#sense")) {
                ...
                let pos = match pos.as_str() {
                    ...
                    _ => return Err(Error::UnknownPartOfSpeech { pos }),
                };
...
mod tests {
    ...
    #[test]
    fn build_dict() {
        let dict = Dictionary::build_from_reader(
            include_str!("./english-wordnet-2022-subset.ttl").as_bytes(),
        )
        .unwrap();
